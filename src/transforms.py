...
def get_to_tensor():
    return A.Compose(
        ...
        bbox_params=A.BboxParams(
            format="pascal_voc", label_fields=["labels", "iscrowd"]
        ),
    )
...
def get_transforms_test(
    ...
    if rotate:
        transforms_list.append(
            A.augmentations.geometric.rotate.Rotate(border_mode=cv2.BORDER_CONSTANT)
        )
    if random_crop:
        transforms_list.append(
            A.augmentations.crops.transforms.RandomCrop(500, 500, p=0.5)
        )
    if blur:
    ...
    transforms_list.extend(
        (A.augmentations.geometric.resize.LongestMaxSize(max_size=1333), ToTensor())
    )
    return A.Compose(
        transforms_list,
        bbox_params=A.BboxParams(
