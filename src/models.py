...
class ObjectDetector(LightningModule):
    def __init__(
        ...
        update_lr_scheduler: bool = False,
        al_data=None,
        al_add: int = 200,
    ):
        super().__init__()
    ...
    def _al_val_step(self, batch):
        ...
        for n in range(10):
            new_x = [
                img + (torch.randn(img.size(), device=img.device) * n / 10) for img in x
            ]
            preds = self(new_x)
            ious.append(
                [
                    torchvision.ops.box_iou(p["boxes"], gt["boxes"])
                    for p, gt in zip(preds, base_preds)
                ]
            )
        ious_mean = []
        for i in range(len(x)):
            ...
            for iou in ious:
                if ious_sum is None:
                    ...
                    for dim in range(2):
                        shape_pad.append(
                            max(ious_sum.shape[dim] - iou[i].shape[dim], 0)
                        )
                        shape_pad_sum.append(
                            max(iou[i].shape[dim] - ious_sum.shape[dim], 0)
                        )
                    ious_sum = F.pad(
                        ious_sum, (0, shape_pad_sum[1], 0, shape_pad_sum[0])
                    )
                    tmp_iou = F.pad(iou[i], (0, shape_pad[1], 0, shape_pad[0]))
                    ious_sum += tmp_iou
            ious_mean.append(ious_sum / len(ious))
        ious_mean_sum = [
            (y["image_id"].item(), torch.sum(ious_mean_img).item())
            for y, ious_mean_img in zip(y, ious_mean)
        ]
